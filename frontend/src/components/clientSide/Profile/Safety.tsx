...
export default function SafetyForm({ user }: { user: UserSelf }) {
  ...
  const save = async () => {
    try {
      await updateUser(user.username, { blurUnsafeEmbeds, defaultSafety, blacklistedTags: blacklistedTags.map((tag) => tag.id) });
  ...
  return (
    ...
    <div className='mb-4'>
      <h2 className="text-xl font-semibold">Safety Preferences</h2>
