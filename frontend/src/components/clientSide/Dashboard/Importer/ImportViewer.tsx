...
export default function ImportViewer() {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      try {
        ...
        setSession(data);
        if (["COMPLETED", "ERROR"].includes(data.status)) clearInterval(interval);
      } catch (err: any) {
  ...
  if (!sessionId) return <div className="p-8 text-red-400">Missing session ID.</div>;
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-4">
          ...
          {session.logs.map((log, i) => {
            ...
            return (
              <div key={i} className="flex gap-2 items-start whitespace-pre-wrap">
                ...
                <span className={`${levelColor} shrink-0 font-semibold`}>[{level}]</span>
      ...
      {session.status === "COMPLETED" && (
        <p className="text-green-400 font-semibold pt-4">
          Import completed successfully.
        </p>
      )}
      {session.status === "ERROR" && (
        <p className="text-red-400 font-semibold pt-4">
